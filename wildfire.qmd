...
```{r}
wildfire_shp_data <- st_read(here("data/LANDSAT_USA_contiguous_and_Hawaii_7d.shp"))
```
...
ggplot(wildfire_counts, aes(x = ACQ_DATE, fill = n_fires)) +
  geom_bar() +
  theme_minimal()
...
wildfire_shp_data %>%
  filter(CONFIDENCE == "high") %>%
  {
    mapboxgl(
      style = mapbox_style("dark"),
      center = c(mean(.$LONGITUDE), mean(.$LATITUDE)),
      zoom = 2
    ) |>
      add_heatmap_layer(
        id = "fire_heatmap",
...
wildfire_area_data <- st_read(
  "data/fire_kml/LANDSAT_USA_contiguous_and_Hawaii_7d_1784060082955.kml",
